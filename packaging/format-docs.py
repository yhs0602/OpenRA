...
def format_docs(version, collectionName, types, relatedEnums):
    ...
    print(f"# {title}\n")
    print(
        f"This documentation is aimed at modders and has been automatically generated for version `{version}` of OpenRA. "
    ...
    print(
        f"Related types with their possible values are listed [at the bottom](#related-value-types-enums)."
    )
    ...
    for namespace in typesByNamespace:
        ...
        for currentType in typesByNamespace[namespace]:
            ...
            if "InheritedTypes" in currentType and currentType["InheritedTypes"]:
                ...
                if inheritedTypes:
                    print(
                        ...
                        + ", ".join(
                            [
                                format_type_name(x, is_known_type(x, types))
                                for x in inheritedTypes
                            ]
                        )
                        + "."
                    )
            ...
            if "RequiresTraits" in currentType and currentType["RequiresTraits"]:
                formattedRequiredTraits = [
                    format_type_name(x, is_known_type(x, types))
                    for x in currentType["RequiresTraits"]
                ]
                print(
                    "\n> Requires trait(s): "
                    + ", ".join(sorted(formattedRequiredTraits))
                    + "."
                )
                ...
                for prop in currentType["Properties"]:
                    ...
                    if prop["InternalType"] in enumNames:
                        ...
                        if prop["InternalType"] in enumReferences:
                            enumReferences[prop["InternalType"]].append(
                                currentType["Name"]
                            )
                        else:
                    ...
                    if "OtherAttributes" in prop:
                        ...
                        print(
                            f'| {prop["PropertyName"]} | {defaultValue} | {typeName} | {prop["Description"]} |'
                        )
                    else:
                        print(
                            f'| {prop["PropertyName"]} | {prop["DefaultValue"] or ""} | {typeName} | {prop["Description"]} |'
                        )
    ...
    if len(relatedEnums) > 0:
        ...
        for relatedEnum in relatedEnums:
            ...
            formattedReferencingTypes = [
                format_type_name(x, is_known_type(x, types))
                for x in distinctReferencingTypes
            ]
